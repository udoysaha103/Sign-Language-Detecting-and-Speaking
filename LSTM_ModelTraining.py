...
if __name__ == "__main__":
    ...
    X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size=0.2, random_state=2, shuffle=True)  # Splitting the data into TRAIN and TEST sets
    ...
    model.fit(X_train, Y_train, epochs=340, callbacks=[tb_callback])   # <-- Training the model for 1000 epochs, changing the number of epochs can be done based on the model performance
    ...
    model.save('LSTM_Model.keras')     # Saving the model in the current directory with the name 'mymodel.keras'